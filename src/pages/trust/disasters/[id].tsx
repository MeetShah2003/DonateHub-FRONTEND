...
import {
  DisasterTransactionForTrust,
  NormalTransactionForTrust,
} from "@/types/types";
...
const DisasterTransaction = () => {
  ...
  const [transactions, setTransactions] =
    useState<DisasterTransactionForTrust>();
  ...
  return (
    <TrustNavbar title="Disaster Transactions">
      <div className="flex flex-col gap-3">
        ...
        {transactions?.myTrust &&
          transactions?.myTrust.length &&
          transactions?.myTrust.map(
            ({ paymentId, transactionDate, uId, donatedAmount }, index) => {
              return (
                <div
                  key={index}
